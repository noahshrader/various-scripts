// ES5-compatible shader background (solid #FBFBF9, no red light)
// - No arrow functions, no const/let, no template literals, no trailing commas
// - Auto-loads three.js r128 if missing
// - Initializes on .patternbg
// - Uses ONLY the light variants (RED LIGHT removed)
...
function pickTwoDistinctLightColors() {
  var i = Math.floor(Math.random() * LIGHT_COLORS.length);
  var j = Math.floor(Math.random() * (LIGHT_COLORS.length - 1));
  if (j >= i) j = j + 1; // ensure j != i
  return [LIGHT_COLORS[i], LIGHT_COLORS[j]];
}
...
var fragmentShader = [
  ...
  "  );",
  "  int x = int(mod(pixelCoord.x, float(dim)));",
  "  int y = int(mod(pixelCoord.y, float(dim)));",
  "  return B[x + y * dim];",
  "}",
  ...
  "  blob1 = mix(blob1, blob1 * 0.05, edgeBias);",
  ...
  // Start from solid #FBFBF9 (0.984314, 0.984314, 0.976471)
  "  vec3 finalColor = vec3(0.984314, 0.984314, 0.976471);",
  "  finalColor = mix(finalColor, uBlob1, blob1Mask * 0.75);",
  "  finalColor = mix(finalColor, uBlob2, blob2Mask * 0.75);",

  // subtle dither to reduce banding (keeps solid base)
  "  float d = getBayerFromCoordLevel(uv * uResolution * 0.5);",
  "  finalColor += vec3(d * 0.02);",
...
// ---------- Core ----------
function ShaderBackground(targetElement) {
...
ShaderBackground.prototype.init = function () {
  ...
  this.renderer.setClearColor(0xfbfbf9, 1.0); // solid base to match page
...
ShaderBackground.prototype.createMesh = function () {
  ...
  // Pick two distinct light colors (no red)
  var pair = pickTwoDistinctLightColors();
  var col1 = hexToVec3(pair[0]);
  var col2 = hexToVec3(pair[1]);
...
ShaderBackground.prototype.onResize = function () {
...
ShaderBackground.prototype.onMouseMove = function (event) {
...
ShaderBackground.prototype.animate = function () {
  ...
  this.mouseVelocity.x =
    (this.targetMouse.x - this.smoothedMouse.x) * smoothFactor;
  this.mouseVelocity.y =
    (this.targetMouse.y - this.smoothedMouse.y) * smoothFactor;
...
function initShaderBackgrounds() {
  var elements = document.getElementsByClassName("patternbg");
  for (var i = 0; i < elements.length; i++) {
    try {
      new ShaderBackground(elements[i]);
    } catch (e) {
      console.warn("Shader init error:", e);
    }
  }
}
...
(function ensureThreeAndInit() {
  if (typeof THREE === "undefined") {
    ...
    s.onload = initShaderBackgrounds;
